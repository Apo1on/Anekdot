...
#include <vector>
using namespace std;
// создает случайное число в диапозионе от min до max
...
// сохраняет массив mas размером n в файл именем way
...
///<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< все теже функции но под матрицу>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
void FillVector(vector<double> mas, const int& n, const int& min, const int& max)
{
	for (int i = 0; i < n; i++)
	{
...
// считает сумму всех элементов
double Summ(const vector<double> mas, const int& n)
{
	double summa = 0.0;
	// идет по массиву складывая значения		
	for (int i = 0; i < n; i++)
